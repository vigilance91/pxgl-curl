...
rem 
if %_CLI_LUM% lss 0 (
    echo invalid greyscale channel, too small
    goto dealloc
) else if %_CLI_LUM% gtr 255 (
    echo invalid greyscale channel, too large
    goto dealloc
)
goto shiftLoop
...
rem 
if not defined _CLI_LUM set /A _CLI_LUM=0
rem 
set "_URL_ARGS=w=%_CLI_WIDTH%&h=%_CLI_HEIGHT%&lum=127"
rem echo "%_URL_ARGS%"
rem 
if defined _CLI_PATH (
    curl --get "https://vigilstudios.ca/pxgl-php56/greyscale/fill.php?%_URL_ARGS%" --output "%_CLI_PATH%" || (
        if not defined _CLI_QUIET (
