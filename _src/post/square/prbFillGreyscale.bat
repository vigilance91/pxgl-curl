...
if defined _CLI_PATH (
    curl -F s=%_CLI_SIZE% "https://vigilstudios.ca/pxgl-php56/square/prbFillGreyscale.php" --output "%_CLI_PATH%" || (
        if not defined _CLI_QUIET (
            echo %_DBG_PREFIX% calling curl https://vigilstudios.ca/ failed with source @ "%_CLI_PATH%"
        )
        goto dealloc
    )
    rem 
